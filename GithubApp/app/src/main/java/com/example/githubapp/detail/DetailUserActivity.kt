...
class DetailUserActivity : AppCompatActivity() {
    ...
    private fun setFavIcon(isfavorite: Boolean){
        binding.fbFavorite?.apply {
            if (isfavorite) {
                setImageDrawable(ContextCompat.getDrawable(this@DetailUserActivity,
    ...
    private fun setDetailUser(Data: DetailUserResponse) {
        ...
        favoriteViewModel.getFavoriteUser().observe(this) {favoriteList ->
            ...
            binding.fbFavorite?.setOnClickListener{
                val isFavorited = Data.name.let {name ->
                    FavoriteUser(
                        id= Data.login,
                        username= name,
                ...
                try {
                    if (isFavorited != null ) favoriteViewModel.AddDelUser(isFavorited,favoriteList.any { it.id == Data.login })
                } catch (e: Exception){
